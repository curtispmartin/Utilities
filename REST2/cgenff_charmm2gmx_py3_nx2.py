...
if(float('.'.join(nx.__version__.split('.')[:2])) < 2.0):
	...
	if(float('.'.join(nx.__version__.split('.')[:2])) > 2.3):
...
#for output
itpfile = mol_name.upper() + ".itp"
prmfile = mol_name.upper() + ".prm"
initpdbfile = mol_name.upper() + "_ini.pdb"
topfile = mol_name.upper() +".top"
